   ...
   public class QueryRepository<TEntity>:IQueryRepository<TEntity>
    {
       ...
       public QueryRepository(string connectionStringName)
       {
       ...
       private QueryRepository(QueryContext context)
       {
        ...
        public IQueryRepository<TEntity> Where(System.Linq.Expressions.Expression<Func<TEntity, bool>> predicate)
        {
            _queryContext.Nodes.Add(new QueryNode(QueryNodeType.Where, predicate) );
            return this;
        }

        public IQueryRepository<TEntity> OrderBy<TKey>(System.Linq.Expressions.Expression<Func<TEntity, TKey>> keySelector)
        {
            _queryContext.Nodes.Add(new QueryNode(QueryNodeType.OrderBy, keySelector));
            return this;
        }

        public IQueryRepository<TEntity> OrderByDesc<TKey>(System.Linq.Expressions.Expression<Func<TEntity, TKey>> keySelector)
        {
            _queryContext.Nodes.Add(new QueryNode(QueryNodeType.OrderByDesc, keySelector));
            return this;
        }

        public IQueryRepository<TResult> Select<TResult>(Expression<Func<TEntity, TResult>> selector)
        {
            // 只允许使用一次select 
            ...
            return new QueryRepository<TResult>(_queryContext);
        }

        //public IQueryRepository<TEntity> Limit(int pageSize = 10, int pageIndex = 0)
        //{
        //    _queryContext.Limit(pageSize, pageIndex); 
        //    return this;
        //}
